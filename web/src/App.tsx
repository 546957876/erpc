      ...
      {currentConfig.isError ? <Alert type="warning" showIcon message="无法读取持久配置，健康参数暂不可编辑" className="mb-5" /> : matchingProjectIndexes.length > 1 ? <Alert type="error" showIcon message="配置中存在重复项目标识，无法安全编辑健康参数" className="mb-5" /> : configProjectIndex >= 0 && <div className="settings-section">
        ...
        >
          <div className="form-grid">
            ...
            <Form.Item name="svmStatePollerDebounce" label={<HealthFieldLabel title="SVM 状态轮询防抖" help="单位：ms（毫秒）。两次 SVM 状态刷新之间的最短间隔。默认 400ms；与 EVM 状态轮询周期独立。" />} extra="单位：ms（毫秒），只填写数字，例如 400。" rules={[{ required: true, message: "请输入数值" }, { type: "number", min: 0.001, message: "请输入大于 0 的数值" }]}><InputNumber min={0.001} step={1} precision={6} addonAfter="ms" className="w-full" /></Form.Item>
          </div>
