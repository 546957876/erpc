...
import { useConfigRevisions, useDeleteConfigRevision, useRestoreConfig, useRuntimeStatus, type ConfigRevision } from "../app/api";

export function RevisionsPage() {
  const revisions = useConfigRevisions();
  const runtime = useRuntimeStatus();
  const restore = useRestoreConfig();
  const remove = useDeleteConfigRevision();
  ...
  async function deleteRevision(revision: number) {
    try {
      await remove.mutateAsync(revision);
      apiMessage.success(`配置版本 v${revision} 已删除`);
    } catch (error) { apiMessage.error(error instanceof Error ? error.message : "删除失败"); }
  }
  const latest = Math.max(0, ...(revisions.data || []).map((item) => item.revision));
  const runtimeRevision = runtime.data?.runningRevision;
  return <section className="page-enter">
    ...
    <Table<ConfigRevision> rowKey="revision" dataSource={revisions.data || []} pagination={false} className="ops-table" columns={[
      ...
      { title: "操作", key: "action", render: (_, row) => <Space size="small">
        <Popconfirm title={`从 v${row.revision} 创建新版本？`} okText="确认" cancelText="取消" onConfirm={() => void restoreRevision(row.revision)}>
          <Button size="small" icon={<RollbackOutlined />} disabled={row.revision === latest || restore.isPending}>恢复为新版本</Button>
        </Popconfirm>
        <Popconfirm title={`删除配置版本 v${row.revision}？`} description="删除后无法恢复；最新版本和运行记录引用的版本不能删除。" okText="删除" okButtonProps={{ danger: true }} cancelText="取消" onConfirm={() => void deleteRevision(row.revision)}>
          <Button size="small" danger icon={<DeleteOutlined />} disabled={row.revision === latest || row.revision === runtimeRevision || remove.isPending}>删除</Button>
        </Popconfirm>
      </Space> },
    ]} />
  </section>;
